	...
	function handler(player, msg)
		...
		for i, v in next, messages.blacklisted do
			if string.match(msg, i) then
				...
				if (getgenv()).autoreport.Advertise == true then (game:GetService("ReplicatedStorage")).DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w " .. player.Name .. " you got mass reported :)", "All"); end;
			end;
		end;
